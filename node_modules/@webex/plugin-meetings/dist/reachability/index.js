    ...
    _this.gatherReachability = function () {
      ...
      try {
        ...
        return _this.reachabilityRequest.getClusters().then(function (clusters) {
          ...
          _loggerProxy2.default.logger.error('Reachability:index#gatherReachability --> Error in calling getClusters(): ' + error);

          return {};
        });
      } catch (error) {
        _loggerProxy2.default.logger.error('Reachability:index#gatherReachability --> Error in accessing LocalStorage: ' + error);
      ...
      return new _promise2.default(function (resolve) {
        _this.getLocalSDPForClusters(clusterList).then(function (localSDPData) {
          if (!localSDPData || !(0, _keys2.default)(localSDPData).length || !localSDPData.localSDPList || !localSDPData.peerConnection) {
            ...
            _loggerProxy2.default.logger.info('Reachability:index#performReachabilityCheck --> Local SDP is empty or has missing elements..returning');
            resolve({});
          }
          ...
          _loggerProxy2.default.logger.error('Reachability:index#performReachabilityCheck --> Error in getLocalSDPForClusters: ' + error);
          resolve({});
        });
      });
    };
      ...
      return new _promise2.default(function (resolve) {
        _this.reachabilityRequest.remoteSDPForClusters(localSDPList).then(function (remoteSDPResponse) {
          ...
          if (!remoteSDPList || !remoteSDPList.length) {
            _loggerProxy2.default.logger.info('Reachability:index#setRemoteOfferForEachCluster --> Remote SDP is empty.');
            ...
            if (jsonObj && jsonObj.sdp) {
              ...
              _this.setIceCallBackHandlers(pc).then(function (iceStatsResult) {
                ...
                if (_this.processedClusterResults.length === remoteSDPList.length) {
                  ...
                  _loggerProxy2.default.logger.info('Reachability:index#setRemoteOfferForEachCluster --> Returning reachability result' + (0, _stringify2.default)(reachabilityResult));
                  resolve(reachabilityResult);
                }
              }).catch(function (error) {
                _loggerProxy2.default.logger.error('Reachability:index#setRemoteOfferForEachCluster --> Error in setIceCallBackHandlers(): ' + error);
                _this.cleanUpPeerConnections(_this.peerConnectionMap);
                resolve({});
              });
              _peerConnectionManager2.default.setRemoteSessionDetails(pc, _constants.ANSWER, jsonObj.sdp);
            } else {
              _loggerProxy2.default.logger.error('Reachability:index#setRemoteOfferForEachCluster --> Error in getting remote sdp.');
          ...
          _loggerProxy2.default.logger.error('Reachability:index#setRemoteOfferForEachCluster --> Error in remoteSDPForClusters!:' + error);
          _this.cleanUpPeerConnections(_this.peerConnectionMap);
          resolve({});
        });
      });
      ...
      return new _promise2.default(function (resolve, reject) {
        clusterList.forEach(function (cluster) {
          ...
          _peerConnectionManager2.default.createOffer(_util2.default.createPeerConnection()).then(function (peerConnection) {
            ...
            _loggerProxy2.default.logger.error('Reachability:index#getLocalSDPForClusters --> Error in getLocalSDP : ' + error);
            reject(error);
          });
        });
      });
    ...
    _this.cleanUpPeerConnections = function (peerConnectionMap) {
      ...
      try {
        for (var _iterator2 = (0, _getIterator3.default)((0, _keys2.default)(peerConnectionMap)), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          ...
          if (pc) {
            _loggerProxy2.default.logger.info('Reachability:index#cleanUpPeerConnections --> Cleaning up peer connection created for cluster : ' + key);
            pc.close();
          }
        }
      } catch (err) {
    ...
    _this.setIceCallBackHandlers = function (pc) {
      ...
      return new _promise2.default(function (resolve) {
        ...
        timeout = window.setTimeout(function () {
          _loggerProxy2.default.logger.warn('Reachability:index#setIceCallBackHandlers --> iceCallBackHandler#' + _this.clusterId + ' ice handler timed out.');
