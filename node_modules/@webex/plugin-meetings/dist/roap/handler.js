    ...
    if (action.msg && action.msg.messageType && action.msg.errorType) {
      if (_util2.default.findError(action.msg.messageType, action.msg.errorType, action.type)) {
        _util2.default.handleError(meeting.mediaProperties.peerConnection).then(function (res) {
          ...
          _loggerProxy2.default.logger.warn('Roap:handler#checkForAndHandleErrors --> Cannot reset the peer connection with error: ' + err);
        });
...
var handleSessionStep = function handleSessionStep(roap, session) {
  ...
  if (session.OFFER && messageType === _constants._OFFER_) {
    ...
    _loggerProxy2.default.logger.warn('Roap:handler#handleSessionStep --> Glare condition occurred with new mercury event, sequenceId: ' + sequenceId);
  } else {
    _loggerProxy2.default.logger.info('Roap:handler#handleSessionStep --> Save OFFER/ANSWER seq:' + sequenceId + ' new mercury event ' + messageType + 'local state: ' + (0, _stringify2.default)(session.state.state, null, 2));
    ...
    value: function perform(session, meeting, action) {
      ...
      switch (session.state.state) {
        ...
        case _constants.ROAP.ROAP_STATE.WAIT_TX_ANSWER:
          ...
          if (_util2.default.shouldHandleMedia(meeting)) {
            _util2.default.updatePeerConnection(meeting, session).then(function (answerSdps) {
              ...
              _loggerProxy2.default.logger.error('Roap:handler#perform --> Error occured during wait receive answer, continuing, ' + err);
            });
          }
          break;
        ...
        case _constants.ROAP.ROAP_STATE.ERROR:
          _loggerProxy2.default.logger.error('Roap:handler#perform --> Roap State ERROR for session: ' + session);
          break;
        case _constants.ROAP.ROAP_STATE.GLARE:
          ...
          _loggerProxy2.default.logger.warn('Roap:handler#perform --> Roap State resolved the GLARE condition.');
          if (session.GLARE_OFFER.tieBreaker < session.OFFER.tieBreaker) {
            // 2
            _loggerProxy2.default.logger.log('Roap:handler#perform --> Roap State local offer won after GLARE.');
          } else {
            _loggerProxy2.default.logger.log('Roap:handler#perform --> Roap State remote offer won after GLARE.');
          }
    ...
    value: function handleAction(session, action, meeting, correlationId) {
      ...
      switch (action.type) {
        ...
        case _constants.ROAP.RECEIVE_CALL_LEAVE:
          ...
          _loggerProxy2.default.logger.log('Roap:handler#handleAction --> RECEIVE_CALL_LEAVE event captured, cleaning up the RoapHandler for correlationId: ' + correlationId);break;
        case _constants.ROAP.RESET_ROAP_STATE:
          _collection2.default.deleteSessionSequence(correlationId, action.msg.seq);
          _loggerProxy2.default.logger.log('Roap:handler#handleAction --> RESET_ROAP_STATE event captured, resetting the RoapHandler state based on sequenceId: ' + action.msg.seq);break;
        default:
          return true;
      }
