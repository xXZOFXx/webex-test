...
var shouldStep = function shouldStep(roap, meeting) {
  ...
  _loggerProxy2.default.logger.log('Roap:state#shouldStep --> RoapStateMachine: PeerConnectionState, ', meeting.mediaProperties.peerConnection.signalingState);
  _loggerProxy2.default.logger.log('Roap:state#shouldStep --> RoapStateMachine: success save proceeding with transition, ', roap.msg);
  ...
  switch (value) {
    ...
    case _constants.ROAP.ROAP_STATE.WAIT_RX_ANSWER:
      if (signal === _constants.ROAP.ROAP_SIGNAL.RX_ANSWER) {
        ...
        _loggerProxy2.default.logger.error('Roap:state#handleTransition --> Race Condition no mediaId, continuing.');
    ...
    var RoapState = _javascriptStateMachine2.default.factory({
      ...
      methods: {
        ...
        onAfterStep: function onAfterStep(transition) {
          _loggerProxy2.default.logger.log('Roap:state#onAfterStep --> RoapStateMachine->onAfterStep#fired! State changed from \'' + transition.from + '\' to \'' + transition.to + '\' with transition \'' + transition.transition + '\'\'.');
        }
      }
    });
