...
var doToggle = function doToggle(transition, audio, meeting) {
  ...
  return _util2.default.remoteUpdateAudioVideo(audio.mute, videoMuted, meeting).then(function () {
    _loggerProxy2.default.logger.log('Meeting:audio#doToggle --> AudioStateMachine->onAfterToggle#' + transition.event + ' fired! State changed from \'' + transition.from + '\' to \'' + transition.to + '\' with transition \'' + transition.transition + '\'\'.');
  }).catch(function (remoteUpdateError) {
    _loggerProxy2.default.logger.log('Meeting:audio#doToggle --> AudioStateMachine->onBeforeToggle#' + transition.event + ' fired! State failed to change with transition \'' + transition.transition + '\'\'. After local audio toggle failed, resetting remote also failed, meeting audio in bad state with error: ' + remoteUpdateError + '.');
