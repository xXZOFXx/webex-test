    ...
    return new _javascriptStateMachine2.default({
      ...
      methods: {
        ...
        onBeforeError: function onBeforeError(transition, error) {
          _loggerProxy2.default.logger.error('Meeting:state#onBeforeError --> state->onError#meeting.id: ' + this.meeting.id + ' | Transition \'' + transition.transition + '\' : ' + transition.from + ' -> ' + transition.to + ' failed after last state transition, with error ' + error + '. Moving to a non-informative state. Last states: ' + this.history);
        },
        ...
        onAfterTransition: function onAfterTransition(transition) {
          _loggerProxy2.default.logger.log('Meeting:state#onAfterTransition --> state->onAfterTransition#meeting.id: ' + this.meeting.id + ' | Transition \'' + transition.transition + '\' : ' + transition.from + ' -> ' + transition.to + ' executed. Last states: ' + this.history);
        }
      },
