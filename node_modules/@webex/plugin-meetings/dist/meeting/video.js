...
var doToggle = function doToggle(transition, video, meeting) {
  ...
  return _util2.default.remoteUpdateAudioVideo(audioMuted, video.mute, meeting).then(function () {
    _loggerProxy2.default.logger.log('Meeting:video#doToggle --> VideoStateMachine->onAfterToggle#' + transition.event + ' fired! State changed from \'' + transition.from + '\' to \'' + transition.to + '\' with transition \'' + transition.transition + '\'\'.');
  }).catch(function (remoteUpdateError) {
    _loggerProxy2.default.logger.log('Meeting:video#doToggle --> VideoStateMachine->onBeforeToggle#' + transition.event + ' fired! State failed to change with transition \'' + transition.transition + '\'\'. After local Video toggle failed, resetting remote also failed, meeting video in bad state with error: ' + remoteUpdateError + '.');
